...
import { placeOrder, removeFromCart } from '../redux/cart/action'
...
function Cart() {
 ...
 const total = cartState.cartProducts.reduce((acc, item) => acc + item.productId.price, 0);
          ...
          <Container maxW='container.lg' mt={8}>
            <Grid templateColumns={"repeat(5, 1fr)"} gap='6'>
              <GridItem colSpan={{base:5,md:3}}>
                ...
                {
                  cartState.cartProducts.map((item,index) => (
                    <Box key={index} mt={2} bg='white' p='6' borderRadius='3px' border='1px solid' borderColor='gray.200' >
                      <Box  borderRadius='3px' bg='white' display='grid' gridTemplateColumns='24% 75%' gridGap='1%'>
                        <Box colSpan='1'>
                          <Image src={item.productId.images[0]} w='100%' />
                        </Box>
                        <Box p='4' colSpan='2'>
                          <Flex justifyContent={'space-between'}>
                            <Text fontSize='16px' fontWeight='bold'>{item.productId.name}</Text>
                            <Box mt='-15px'>
                              <FontAwesomeIcon position='relative' fontSize={22} color='gray' icon={faXmark} cursor='pointer' onClick={() => { onOpen(); setProduct(item) }} />
                            </Box>
                          </Flex>
                          <Text fontSize='13px' mt={1} fontWeight='thin'>{item.productId.tagline}</Text>
                          <Text fontSize='11px' mt={1} fontWeight='thin'>Sold By: Omnitech Retail</Text>
                          <Flex gap={1} alignItems='center'>
                            <Text fontWeight='bold' fontSize='16px' mt='1'>Rs. {item.productId.price}</Text>
                            <Text fontWeight='thin' as={'s'} fontSize='13px' color='gray' mt='6px'>Rs. {item.productId.mrp}</Text>
                            <Text fontWeight='thin' fontSize='13px' color='orange.400' mt='6px'>({((item.productId.price * 100) / item.productId.mrp).toFixed(0)}% OFF)</Text>
                          </Flex>
              ...
              <GridItem colSpan={{base:5,md:2}}>
                ...
                <Box mt={4} bg='white' p='6' borderRadius='3px' border='1px solid' borderColor='gray.200' >
                  <Text fontSize='14px' fontWeight='bold'>Price Details ({cartState.cartProducts.length} Items)</Text>
                  <Flex justifyContent='space-between' mt={4}>
                    <Text fontSize='14px' fontWeight='thin'>Total MRP</Text>
                    <Text fontSize='14px' fontWeight='thin'>Rs. {cartState.cartProducts.reduce((acc, item) => acc + item.mrp, 0)}</Text>
                  </Flex>
                  <Flex justifyContent='space-between' mt={3}>
                    <Text fontSize='14px' fontWeight='thin'>Discount on MRP</Text>
                    <Text fontSize='14px' color='teal' fontWeight='thin'>- Rs. {cartState.cartProducts.reduce((acc, item) => acc + item.mrp, 0) - (total)}</Text>
                  </Flex>
                  <Flex justifyContent='space-between' mt={3}>
